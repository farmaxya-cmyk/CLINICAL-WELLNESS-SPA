...
// --- ASSETS AUDIO (Base64 WAV forniti) ---
const DING_INHALE_BASE64 = "";
const DING_EXHALE_BASE64 = "";
...
const BreathingScreen = () => {
    ...
    // Refs
    const audioRef = React.useRef<HTMLAudioElement>(null); // Background music player
    const audioCtxRef = React.useRef<AudioContext | null>(null);
    const masterGainRef = React.useRef<GainNode | null>(null);
    const modeGainRef = React.useRef<GainNode | null>(null); // Gate per i Ding
    
    // Buffers per i suoni pre-caricati
    ...
    const loadBuffers = async (ctx: AudioContext) => {
        // Carica Inhale
        if (!dingInhaleBuffer.current) {
            try {
                const arrayBuffer = base64ToArrayBuffer(DING_INHALE_BASE64);
                dingInhaleBuffer.current = await ctx.decodeAudioData(arrayBuffer);
            } catch (e) { console.error("Error decoding inhale buffer", e); }
        }
        // Carica Exhale
        if (!dingExhaleBuffer.current) {
            try {
                const arrayBuffer = base64ToArrayBuffer(DING_EXHALE_BASE64);
                dingExhaleBuffer.current = await ctx.decodeAudioData(arrayBuffer);
            } catch (e) { console.error("Error decoding exhale buffer", e); }
        }
    };

    // Gestione Volume Musica (Tag Audio HTML standard)
    React.useEffect(() => {
        if (audioRef.current) {
    ...
    // Gestione Traccia Musicale
    React.useEffect(() => {
        if (audioRef.current) {
            const audioEl = audioRef.current;
            if (audioEl.src !== musicTrack && !audioEl.src.endsWith(musicTrack)) {
    ...
    // Gate Control (Volume Ding in tempo reale tramite WebAudio API)
    React.useEffect(() => {
        if (audioCtxRef.current && modeGainRef.current) {
            const ctx = audioCtxRef.current;
            const targetGain = mode === 'closed' ? dingVolume : 0;
            // Transizione fluida per evitare click
            modeGainRef.current.gain.cancelScheduledValues(ctx.currentTime);
            modeGainRef.current.gain.setTargetAtTime(targetGain, ctx.currentTime, 0.1);
        }
    }, [mode, dingVolume, isActive]);

    // --- AUDIO SCHEDULING (BUFFER BASED) ---
    
    // Schedula un buffer (file audio) nel futuro preciso usando il clock hardware
    const scheduleSound = (buffer: AudioBuffer | null, time: number, ctx: AudioContext, dest: AudioNode) => {
        if (!buffer) return;
        
        const source = ctx.createBufferSource();
        source.buffer = buffer;
        source.connect(dest);
        source.start(time); // .start(time) è la chiave per il funzionamento in background
        scheduledNodesRef.current.push(source);
    };

    const scheduleFullSession = () => {
        if (!audioCtxRef.current || !modeGainRef.current) return;
        const ctx = audioCtxRef.current;
        const dest = modeGainRef.current; // Connette al "Gate" (controllo volume)
        const pattern = BREATHING_PATTERNS[patternKey];
        
        const now = ctx.currentTime;
        let cursorTime = now + 0.1; // Piccolo buffer iniziale
        const endTime = now + duration;

        stopAudioEngine(); // Pulisce eventuali vecchi nodi

        // Loop che calcola tutti i tempi futuri e invia i comandi al chip audio
        while (cursorTime < endTime) {
            // 1. Suono Inspirazione
            scheduleSound(dingInhaleBuffer.current, cursorTime, ctx, dest);
            cursorTime += pattern.inhale;

            // 2. Trattieni (Nessun suono)
            if (pattern.hold > 0) {
            ...
            if (cursorTime < endTime) {
                scheduleSound(dingExhaleBuffer.current, cursorTime, ctx, dest);
            }
    ...
    const handleStart = async () => {
        // 1. Setup Context & Graph solo al primo click (Policy Browser)
        if (!audioCtxRef.current) {
            ...
            modeGainRef.current = ctx.createGain(); // The Gate
            
            modeGainRef.current.connect(masterGainRef.current);
            masterGainRef.current.connect(ctx.destination);
            
            masterGainRef.current.gain.value = 1.0;
            modeGainRef.current.gain.value = mode === 'closed' ? dingVolume : 0;
        }

        const ctx = audioCtxRef.current;

        // 2. Resume Context se sospeso
        if (ctx.state === 'suspended') {
            await ctx.resume();
        }

        // 3. Carica i Buffer (Decodifica WAV)
        await loadBuffers(ctx);

        setIsActive(true);

        // 4. Avvia Musica di Fondo (ESSENZIALE PER MANTENERE ATTIVO IL THREAD SU MOBILE)
        if (audioRef.current) {
            audioRef.current.volume = musicVolume;
            audioRef.current.play().catch(e => alert("Tocca lo schermo per avviare l'audio"));
        }
    };
    ...
    // Main Effect: Avvia la schedulazione quando isActive diventa true
    React.useEffect(() => {
        if (isActive) {
            // Schedula l'Audio (Hardware Clock - Preciso e Background-Safe)
            scheduleFullSession();
            
            // Schedula i Visual (JS Clock - Potrebbe fermarsi in background, ma si riallinea)
            const pattern = BREATHING_PATTERNS[patternKey];
    ...
    return (
        <div className="p-4 md:p-8 animate-fade-in flex flex-col items-center justify-center min-h-full pb-32">
            ...
            {/* Background Music Player - Mantiene attivo il thread audio su mobile */}
            <audio 
                ref={audioRef} 
                loop
                preload="auto"
